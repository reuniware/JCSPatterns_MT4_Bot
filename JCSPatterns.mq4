   ...
   else
     {
      if(LastBarTime==-1)
      ...
      else
        {
         ...
         if(open_array[1]<close_array[1] && open_array[2]>close_array[2])
           {
            if(close_array[1]>open_array[2] && open_array[1]<close_array[2])
              {
               rsi=iRSI(NULL,0,14,PRICE_CLOSE,0);

               log("Time = "+Time[1]+" ; Avalement haussier ; RSI = "+rsi);

               if(NombreDePositionAcheteuses()==0 /*&& rsi<45*/)
                 {
                  log("Ordre d'achat");
                  ticket=OrderSend(Symbol(),OP_BUY,2,Ask,3,/*Ask-0.003*/0,Bid+0.1,"JCS",16384,0,Green);
                 }
...
//log("Ordre d'achat: stoploss="+DoubleToString(stoploss));
   ticket=OrderSend(Symbol(),OP_BUY,Lots,Ask,3,stoploss,0,"JCS",16384,0,Green);
   if(ticket>0)
     {
...
//log("Ordre de vente: stoploss="+DoubleToString(stoploss));
   ticket=OrderSend(Symbol(),OP_SELL,Lots,Bid,3,stoploss,0,"JCS",16384,0,Red);
   if(ticket>0)
     {
      if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES))
        {
   ...
   for(count=0; count<OrdersTotal(); count++)
     {
      ...
      if(OrderType()<=OP_SELL && // check for opened position 
         OrderSymbol()==Symbol()) // check for symbol
        {
         ...
         if(OrderType()==OP_BUY)
           {
            ...
            if(orderProfit>0)
              {
               if(ProfitPerteTotale<0)
                 {
                  if(orderProfit>=MathAbs(ProfitPerteTotale))
                    {
                     log("FermerLesPositionsAcheteuses: Le PnL de la position est positif et rattrape les pertes totales cumulées");
                     Lots=InitialLots;
                     ProfitPerteTotale=0;
                    }
                  else
                    {
                     log("FermerLesPositionsAcheteuses: Le PnL de la position est positif mais ne rattrape pas les pertes totales cumulées");
                     Lots=Lots+InitialLots;
                     ProfitPerteTotale=ProfitPerteTotale+orderProfit;
                    }
                 }
               else if(ProfitPerteTotale>=0)
                 {
                  log("FermerLesPositionsAcheteuses: Le PnL de la position est positif et les pertes totales cumulées sont positives");
                  Lots=InitialLots;
                  ProfitPerteTotale=0;
                 }
              }
              ...
              {
               log("FermerLesPositionsAcheteuses: Le PnL de la position est négatif");
               Lots=Lots+InitialLots;
               ProfitPerteTotale=ProfitPerteTotale+orderProfit;
              }
         ...
         if(JournaliserProfitsPertesBalanceInitiale) log("FermerLesPositionsAcheteuses: Profit/Perte par rapport à la balance initiale = "+DoubleToString(AccountBalance()-initialBalance));
         return;
        }
     }
  }
   ...
   for(count=0; count<OrdersTotal(); count++)
     {
      ...
      if(OrderType()<=OP_SELL && // check for opened position 
         OrderSymbol()==Symbol()) // check for symbol
        {
         ...
         if(OrderType()==OP_SELL)
           {
            ...
            if(orderProfit>0)
               ...
               if(ProfitPerteTotale<0)
                 {
                  if(orderProfit>=MathAbs(ProfitPerteTotale))
                    {
                     log("FermerLesPositionsVendeuses: Le PnL de la position est positif et rattrape les pertes totales cumulées");
                     Lots=InitialLots;
                     ProfitPerteTotale=0;
                    }
                  else
                    {
                     log("FermerLesPositionsVendeuses: Le PnL de la position est positif mais ne rattrape pas les pertes totales cumulées");
                     Lots=Lots+InitialLots;
                     ProfitPerteTotale=ProfitPerteTotale+orderProfit;
                    }
                 }
                 ...
                 {
                  log("FermerLesPositionsVendeuses: Le PnL de la position est positif et les pertes totales cumulées sont positives");
                  Lots=InitialLots;
                  ProfitPerteTotale=0;
                 }
              }
              ...
              {
               log("FermerLesPositionsVendeuses: Le PnL de la position est négatif");
               Lots=Lots+InitialLots;
               ProfitPerteTotale=ProfitPerteTotale+orderProfit;
              }
         ...
         if(JournaliserProfitsPertesBalanceInitiale) log("FermerLesPositionsVendeuses: Profit/Perte par rapport à la balance initiale = "+DoubleToString(AccountBalance()-initialBalance));
         return;
        }
     }
  }
